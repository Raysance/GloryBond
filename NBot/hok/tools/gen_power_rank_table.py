def merge_power_rows(rows, sameuser, namenick=None):
    """Merge per-account power rows into main accounts by sameuser mapping."""
    ...
    for main, item in merged.items():
        ...
        for a in aliases:
            ...
            if a not in alias_to_main:
                continue
            value = main_sub_totals.get(main, {}).get(a, 0.0)
            subs.append(f"({namenick.get(a, a)} {round(value, 1)})")
        out.append(
            {
...
def gen(rows, games, save_path: str, title: str = "底蕴排行"):
    """Generate neon glow power rank matrix image."""
    import os
    import math
    ...
    def pick_color(v: float, avg: float, max_above: float, max_below: float):
        if v >= avg:
            ...
            return (46, 204, 113), math.sqrt(strength) * 0.45, "green"
        denom = max_below if max_below > 0 else 1.0
        strength = clamp01((avg - v) / denom)
        return (231, 76, 60), math.sqrt(strength) * 0.35, "red"
    ...
    def render_glow_tile(w: int, h: int, color_rgb, strength: float):
        ...
        inner_alpha = int(10 + 35 * strength)
        edge_alpha = int(45 + 95 * strength)
        blur_r = 8 + int(7 * strength)
